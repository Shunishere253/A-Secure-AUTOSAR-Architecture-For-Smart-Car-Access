...
#define CLOCK_IP_RUN_POWER_MODE_STATUS                     1U
...
#if (defined (CLOCK_IP_DERIVATIVE_002) || defined(CLOCK_IP_DERIVATIVE_001) || defined(CLOCK_IP_DERIVATIVE_014))
    ...
    defined(CLOCK_IP_DERIVATIVE_010) || defined(CLOCK_IP_DERIVATIVE_009) || defined(CLOCK_IP_DERIVATIVE_011) || defined(CLOCK_IP_DERIVATIVE_013) || defined(CLOCK_IP_DERIVATIVE_015))
...
static const Clock_Ip_NameType ClockSource[CLOCK_IP_SCS_SELECTOR_HARDWARE_VALUES_NO] =
{
    ...
    #endif
    CLOCK_IS_OFF,                  /* clock name for 7  hardware value */
};
...
void SetSimLpoclksRegister_TrustedCall(Clock_Ip_ClockConfigType const *Config)
{
    ...
    for (Index = 0U; Index < Config->GatesCount; Index++)    /* Set clock gates that are under clock control. */
    {
        ...
        {
            SimLpoValue = (SimLpoValue & (~SIM_LPOCLKS_LPO32KCLKEN_MASK)) | (((uint32)(Config->Gates[Index].Enable) << SIM_LPOCLKS_LPO32KCLKEN_SHIFT) & SIM_LPOCLKS_LPO32KCLKEN_MASK);
        }

         /* Gate for LPO_1K_CLK */
        if (LPO_1K_CLK == Config->Gates[Index].Name)
        {
            SimLpoValue = (SimLpoValue & (~SIM_LPOCLKS_LPO1KCLKEN_MASK)) | (((uint32)(Config->Gates[Index].Enable) << SIM_LPOCLKS_LPO1KCLKEN_SHIFT) & SIM_LPOCLKS_LPO1KCLKEN_MASK);
        }
    }
...
static const Clock_Ip_SelectorConfigType *getSelectorConfig(Clock_Ip_NameType Name)
{
    ...
    if (ReturnValue == NULL_PTR)
    {
        ...
        {
            case SCS_RUN_CLK:
                SelectorConfigurations[SelectorConfigIndex].Value = ClockSource[((IP_SCG->RCCR & SCG_RCCR_SCS_MASK) >> SCG_RCCR_SCS_SHIFT) & 0x7U];
                break;
            case SCS_VLPR_CLK:
...
{
    (void) PowerMode;
    if(Notification != CLOCK_MONITOR_NOTIFICATIONS)
    {
        /* Invalid power mode */
        switch(PowerMode)
        {
    ...
    #endif
                    /* #Shut Down SOSC*/
                    Clock_Ip_pxSoscClock->Reset(Clock_Ip_pxSoscConfig);
                    /* #Shut Down FIRC*/
