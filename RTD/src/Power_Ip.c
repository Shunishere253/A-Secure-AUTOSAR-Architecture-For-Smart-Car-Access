...
static void Power_Ip_PrepareLowPowerMode(Power_Ip_PowerModeType PowerMode)
{
    ...
    if ((POWER_IP_VLPS_MODE == PowerMode) || (POWER_IP_VLPR_MODE == PowerMode))
    {
        ...
        * The function Clock_Ip_PowerNotifications will handle disabling the clock
        * before entering low power modes: VLPS and VLPR */
        Clock_Ip_PowerNotifications(VLPR_MODE, BEFORE_POWER_MODE_CHANGE);
...
void Power_Ip_SetMode(const Power_Ip_ModeConfigType * ModeConfigPtr)
{
    ...
    else
    {
        ...
        Clock_Ip_PowerNotifications(ModeChanged,ModeChangeStatus);
  #endif
#endif
    }
}
