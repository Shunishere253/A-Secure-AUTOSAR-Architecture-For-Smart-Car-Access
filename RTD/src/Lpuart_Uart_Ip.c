...
#define LPUART_UART_IP_SW_MAJOR_VERSION_C               3
...
Lpuart_Uart_Ip_StatusType Lpuart_Uart_Ip_SyncReceive(const uint8 Instance, uint8 *RxBuff, const uint32 RxSize, const uint32 Timeout)
{
    ...
    if (!IsReturn)
    {
        ...
        /* Flush the Rx Buffer, all data that is stored in the receive FIFO/buffer to be flushed */
...
Lpuart_Uart_Ip_StatusType Lpuart_Uart_Ip_AbortReceivingData(const uint8 Instance)
{
    ...
    if (UartState->IsRxBusy)
    {
        ...
        /* Flush the Rx Buffer */
        Lpuart_Uart_Ip_FlushRxBuffer(Base);
        /* If timeout occurs, the error report will be raised */
...
static Lpuart_Uart_Ip_StatusType Lpuart_Uart_Ip_StartSendDataUsingInt(const uint8 Instance, const uint8 * TxBuff, const uint32 TxSize)
{
    ...
    /* Enable tx empty interrupt, jump into to interrupt function to push data to DATA register */
    /* The interrupt flag always set because DATA register is empty, only clear when writing data to register. */
...
static void Lpuart_Uart_Ip_CompleteReceiveDataUsingInt(const uint8 Instance)
{
...
#endif
    {
        ...
        /* In Abort case, the receiving need to stop instantly */
        if (LPUART_UART_IP_STATUS_ABORTED == UartState->ReceiveStatus)
        {
            Lpuart_Uart_Ip_StartTimeout(&StartTime, &TimeoutTicks, LPUART_UART_IP_TIMEOUT_VALUE_US, LPUART_UART_IP_TIMEOUT_TYPE);
            /* After disable receiver, writing 0 to RE bit. This RE bit remains 1 until the receiver finishes receiving the current character  */
            /* So the while loop uses to make sure that RE bit was set to 0, if not, The Timeout error will be reported, meaning Reception has not fully stopped */
            while (Lpuart_Uart_Ip_GetReceiverCmd(Base) && \
                    !Lpuart_Uart_Ip_CheckTimeout(&StartTime, &ElapsedTicks, TimeoutTicks, LPUART_UART_IP_TIMEOUT_TYPE) \
                  )
            {}
