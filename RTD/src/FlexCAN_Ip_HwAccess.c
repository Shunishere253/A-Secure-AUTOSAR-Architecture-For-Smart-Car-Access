...
#if (defined (S32K116) || defined (S32K118) || defined(S32K116V) || defined(S32K118V) || defined (CPU_CORTEX_R52))
...
inline static uint32 FlexCAN_DataTransferTxMsgBuff(volatile uint32 * MbData32,
                                                   ...
                                                   const uint8 * MsgData);
#endif /* if (defined (S32K116) || defined (S32K118) || defined(S32K116V) || defined(S32K118V) || defined (CPU_CORTEX_R52)) */
...
boolean FlexCAN_IsFDAvailable(const FLEXCAN_Type * pBase)
{
    ...
    boolean ReturnValue = FALSE;
    for (Idx = 0U; Idx < CAN_FEATURE_FD_INSTANCES; Idx++)
    {
        if (pBase == FlexcanBase[Idx])
        {
            ReturnValue = TRUE;
                       ...
                       )
{
    ...
    uint32 MbWord;
#if (defined (CPU_S32K116) || defined (CPU_S32K118) || defined(S32K116V) || defined(S32K118V) || defined (CPU_CORTEX_R52))
    uint8 Index;
    uint32 Temp;
    const uint8 * pTemp;
#endif
...
#endif /* if (defined (CPU_S32K116) || defined (CPU_S32K118) || defined(S32K116V) || defined(S32K118V) || defined (CPU_CORTEX_R52)) */
    {
        for (Idx = 0U; Idx < (PayloadSize & ~3U); Idx += 4U)
        {
            MbWord = FlexcanMbData32[Idx >> 2U];
...
#endif /* (defined(S32K116) || defined (S32K118) || defined(S32K116V) || defined(S32K118V) || defined (CPU_CORTEX_R52)) */
...
void FlexCAN_ClearOutputLegacyFIFO(FLEXCAN_Type * pBase)
{
    ...
    * the RxFIFO need to be empty to activate DMA */
    if ((uint8)1U == FlexCAN_GetBuffStatusFlag(pBase, FLEXCAN_IP_LEGACY_RXFIFO_FRAME_AVAILABLE))
    {
        ...
        if ((uint32)FLEXCAN_MCR_DMA_MASK == (pBase->MCR & ((uint32)FLEXCAN_MCR_DMA_MASK)))
        {
            ...
            while (((uint8)1U == FlexCAN_GetBuffStatusFlag(pBase, FLEXCAN_IP_LEGACY_RXFIFO_FRAME_AVAILABLE)) && (Idx <= (uint8)12U)); /* avoid blocking */
        }
...
{
    #ifdef FLEXCAN_IP_TIMESTAMP_HR_SOURCE_TBS_USED
        FLEXCAN_IP_TIMESTAMP_REG(Instance) &= ~(FLEXCAN_IP_TIMESTAMP_EN_MASK | FLEXCAN_IP_TIMESTAMP_SEL_MASK);
        FLEXCAN_IP_TIMESTAMP_REG(Instance) |= FLEXCAN_IP_TIMESTAMP_SEL(Config->hrSrc)|FLEXCAN_IP_TIMESTAMP_EN_MASK;
    #else
