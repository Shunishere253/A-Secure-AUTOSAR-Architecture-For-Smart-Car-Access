...
#define CAN_43_FLEXCAN_IPW_SW_MAJOR_VERSION_C             3
...
static void Can_Ipw_InitRxMsgBuffer(const Can_43_FLEXCAN_ControllerConfigType * Can_pController, const Can_43_FLEXCAN_HwObjectConfigType * Can_pHwObject)
{
    ...
    (void)FlexCAN_Ip_ConfigRxMb(Can_pController->Can_u8ControllerOffset, Can_pHwObject->Can_u8HwBufferIndex, &DataInfo, Can_pHwObject->Can_pHwFilterConfig[0U].Can_u32HwFilterCode);
    /* Already in freeze mode */
    (void)FlexCAN_Ip_SetRxIndividualMask(Can_pController->Can_u8ControllerOffset, Can_pHwObject->Can_u8HwBufferIndex, Can_pHwObject->Can_pHwFilterConfig[0U].Can_u32HwFilterMask);
}
...
)
{
    ...
    while ((u8ObjIdx < Can_pHwObjectConfig->Can_u8ObjectCount) && (TRUE == returnResult))
    {
        ...
        Can_Ipw_apxStatus[Can_pControllerConfig->Can_u8ControllerID].mbs[FLEXCAN_IP_MB_HANDLE_RXFIFO].state = FLEXCAN_MB_IDLE;
...
static void Can_Ipw_GetTimestamp(Can_TimeStampType * TimestampPtr, uint32 Value)
{
    ...
    */
    ...
    if (u32TimerCurTimeLow < Value)
    {
        u32TimerCurTimeHigh--;
    }
    u64TimerCurTime = (uint64)((uint64)u32TimerCurTimeHigh << 32U) | (uint64)Value;
    /* Calculate second part */
    TimestampPtr->seconds = (uint32)(u64TimerCurTime / (uint64)CAN_43_FLEXCAN_TS_STIMER_TICK_PER_SECOND);
...
Std_ReturnType Can_43_FLEXCAN_Ipw_SetControllerToStopMode(const Can_43_FLEXCAN_ControllerConfigType * Can_pControllerConfig)
{
    ...
    */
    ...
    for (u8HwObjRefIdx = 0U; u8HwObjRefIdx < Can_pControllerConfig->Can_u8HwObjectRefCount; u8HwObjRefIdx++)
    {
        ...
        if (CAN_43_FLEXCAN_TX_NORMAL == Can_pHwObject->Can_eReceiveType)
        {
            u8Idx = 0U;
            do
            {
    ...
    if (CAN_43_FLEXCAN_TX_NORMAL == Can_pHwObjectConfig->Can_eReceiveType)
    {
        ...
        do
        {
            ...
            if (FLEXCAN_STATUS_BUSY != eRetVal)
            {
                ...
                /* Start transfer data */
                eRetVal = Can_Ipw_TransferData(Can_pControllerConfig, Can_pHwObjectConfig, (Can_pHwObjectConfig->Can_u8HwBufferIndex+u8ObjIdx), PduInfo, &DataInfo);
    ...
    */
    ...
    )
    {
        ...
        if (CAN_43_FLEXCAN_RX_NORMAL == Can_pHwObjectConfig->Can_eReceiveType)
        {
            ...
            Can_Ipw_apxStatus[Can_pControllerConfig->Can_u8ControllerID].mbs[Can_pHwObjectConfig->Can_u8HwBufferIndex].state = FLEXCAN_MB_IDLE;
...
void Can_43_FLEXCAN_Ipw_ProcessPN(const Can_43_FLEXCAN_ControllerConfigType * Can_pControllerConfig, const Can_43_FLEXCAN_IcomConfigsType * pIcomConfig)
{
    ...
    #if (CAN_43_FLEXCAN_LPDU_CALLOUT_SUPPORT == STD_ON)
    #if (CAN_43_FLEXCAN_LPDU_HRH_EXTEND_SUPPORT == STD_ON)
        if (TRUE == CAN_43_FLEXCAN_LPDU_CALLOUT_FUNC_CALLED((uint16)pIcomConfig->u8CanIcomConfigId, \
                                                          CanIf_Mailbox.CanId, \
