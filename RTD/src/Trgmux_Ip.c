...
#include "Mcl_MemMap.h"
        ...
        #define Trgmux_Ip_HwAcc_Init(pTrgmux,TrgmuxCount) \
                (OsIf_Trusted_Call_Return2param(hwAcc_Init_Uint, pTrgmux,TrgmuxCount))
        ...
        #define Trgmux_Ip_HwAcc_GetLockForOutput(pTrgmux, Output) \
                (OsIf_Trusted_Call_Return2param(hwAcc_GetLockForOutput_Uint, pTrgmux, Output))
    #else
        #define Trgmux_Ip_HwAcc_Init(pTrgmux,TrgmuxCount) \
...
#if (TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE == STD_ON)
static Trgmux_Ip_StatusType Static_Trgmux_Ip_ValidateMultiPartitionChannelCall(uint8 UserId);
#endif /* TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE == STD_ON */

uint32 hwAcc_Init_Uint(TRGMUX_Type * const pTrgmux, uint8 TrgmuxCount);
uint32 hwAcc_GetLockForOutput_Uint(const TRGMUX_Type * const pTrgmux, const uint32 Output);
/*==================================================================================================
*                                         LOCAL FUNCTIONS
==================================================================================================*/
#if (STD_ON == TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE)
static Trgmux_Ip_StatusType Static_Trgmux_Ip_ValidateMultiPartitionChannelCall(uint8 UserId)
{
    Trgmux_Ip_StatusType Valid = TRGMUX_IP_STATUS_WRONG_PARTITION;
    uint8 CurrentUserId = (uint8)OsIf_GetUserId();
...
#endif /* STD_ON == TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE */

/**
* @brief            This function is a wrapper around hwAcc_Init.
* @details          This routine returns the hardware init status in uint32 format.
*
* @return           The hardware init status.
* @retval           uint32.
*/
uint32 hwAcc_Init_Uint(TRGMUX_Type * const pTrgmux, uint8 TrgmuxCount)
{
    /* Returns the initial status in uint32 format */
    return (uint32)hwAcc_Init(pTrgmux, TrgmuxCount);
}

/**
* @brief            This function is a wrapper around hwAcc_GetLockForOutput.
* @details          This routine returns the lock status in uint32 format.
*
...
uint32 hwAcc_GetLockForOutput_Uint(const TRGMUX_Type * const pTrgmux, const uint32 Output)
{
    ...
    if (TRUE == hwAcc_GetLockForOutput(pTrgmux, Output))
    {
        Status = 1UL;
    }
    else
    ...
    /* Returns the lock status in uint32 format */
    return Status;
}
...
Trgmux_Ip_StatusType Trgmux_Ip_Init(const Trgmux_Ip_InitType * const pxTrgmuxInit)
{
    ...
    #endif /* #if (STD_ON == TRGMUX_IP_USER_MODE_SUPPORT_IS_AVAILABLE) */
#else
        if(TRGMUX_IP_STATUS_SUCCESS != Trgmux_Ip_HwAcc_Init(LocTrgmuxBase,TRGMUX_IP_TRGMUXn_INSTANCE_COUNT(LocHwInst)))
...
#if (STD_ON == TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE)
    uint8 UserId = Trgmux_Ip_pxInit->paxLogicTrigger[LogicTrigger]->UserId;
#endif /* STD_ON == TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE */
    TRGMUX_Type * LocTrgmuxBase = Trgmux_Ip_paxBaseInst[LocHwInst];
    Trgmux_Ip_StatusType Status = TRGMUX_IP_STATUS_SUCCESS;

#if (STD_ON == TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE)
    Status = Static_Trgmux_Ip_ValidateMultiPartitionChannelCall(UserId);
    if(TRGMUX_IP_STATUS_SUCCESS == Status)
    {
#endif /* STD_ON == TRGMUX_IP_MULTIPARTITION_IS_AVAILABLE */
    #if (STD_ON == TRGMUX_IP_USER_MODE_SUPPORT_IS_AVAILABLE)
        if(0UL != Trgmux_Ip_HwAcc_GetLockForOutput(LocTrgmuxBase, LocOutput))
    #else
