...
#define CLOCK_IP_MONITOR_SW_MAJOR_VERSION_C               3
...
#ifdef CLOCK_IP_CMU_FC_FCE_REF_CNT_LFREF_HFREF
#define CLOCK_IP_CMU_FC_VAR                                     3UL
    ...
    if ((NULL_PTR != Config) && (Config->Enable != 0U))
    {
        ...
        if (HashCmu[Index] != ((((uint32)Config->MonitoredClockFrequency)  ^ ((uint32)Config->Name))))
        {
            ...
            if ((ReferenceClk != 0U) && (BusClk != 0U) && (MonitoredClk != 0U))
            {
                ...
                /* Cmp2 = ceiling of (6 + (3 * fRef / fMonitor)) */
                Cmp2 = 7U + (uint32)(((3U * ReferenceClk) - 1U) / MonitoredClk);
                ...
                switch(Config->Latency)
                {
                    ...
                    case CLOCK_IP_VERY_HIGH_LATENCY_1MS:
                    {
                        ...
                        RefCount = ReferenceClk / (1000000U / Config->Latency);
                ...
                /* Formula for Hfref
                   ceil((fMonitoredClk mul MonitoredClockPlusVariation mul RefCount) div 
                        (fReferenceClk mul ReferenceClockMinusVariation)) 
                         + CLOCK_IP_CMU_FC_VAR
                 */
                HfRef = (((((uint64)MonitoredClk) * ((uint64)(10000U + Config->Variation)) * ((uint64)RefCount)) - 1UL)       \
                /                                                                                                             \
                (((uint64)ReferenceClk) * ((uint64)(10000U - CmuInformation->FreqRefVariation)))) + 1UL                       \
                + CLOCK_IP_CMU_FC_VAR;
                ...
                /* Formula for LfRef
                   floor((fMonitoredClk mul MonitoredClockMinusVariation mul RefCount) div 
                        (fReferenceClk mul ReferenceClockPlusVariation)) 
                         - CLOCK_IP_CMU_FC_VAR
                 */
                LfRef = ((((uint64)MonitoredClk) * ((uint64)(10000U - Config->Variation)) * ((uint64)RefCount))               \
                /                                                                                                             \
                (((uint64)ReferenceClk) * ((uint64)(10000U + CmuInformation->FreqRefVariation))))                             \
                - CLOCK_IP_CMU_FC_VAR;
