...
#define CAN_43_FLEXCAN_SW_MAJOR_VERSION_C               3
...
==================================================================================================*/
    ...
    */
    static void Can_MainFunction_MultipleWrite(uint8 u8WritePoll)
    {
        ...
        #if (CAN_43_FLEXCAN_USE_CANEXCEL_IP  == STD_ON)
            HwObjectID = (Can_apxConfig[u32PartitionId])->CanXL_uHthFirstIndex;
            while (HwObjectID < CAN_43_FLEXCAN_XL_HWOBJECT_CONFIG_COUNT)
            {
                u8ControllerID = (Can_apxConfig[u32PartitionId])->CanXL_pHwObjIDToCtrlIDMap[HwObjectID];
                Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
                if ((TRUE == Can_pController->Can_bActivation) && (u8WritePoll == (Can_apxConfig[u32PartitionId])->CanXL_pHwObjectConfig[HwObjectID].Can_MainFuncPeriodIndex))
                {
                    CanXL_43_FLEXCAN_Ipw_MainFunction_Write(Can_pController, &((Can_apxConfig[u32PartitionId])->CanXL_pHwObjectConfig[HwObjectID]));
                }
                HwObjectID++;
            }
    ...
    */
    ...
    #if (CAN_43_FLEXCAN_USE_CANEXCEL_IP  == STD_ON)
            HwObjectID = 0U;
            while (HwObjectID < (Can_apxConfig[u32PartitionId])->CanXL_uHthFirstIndex)
            {
                u8ControllerID = (Can_apxConfig[u32PartitionId])->CanXL_pHwObjIDToCtrlIDMap[HwObjectID];
                Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
                if ((TRUE == Can_pController->Can_bActivation) && (u8ReadPoll == (Can_apxConfig[u32PartitionId])->CanXL_pHwObjectConfig[HwObjectID].Can_MainFuncPeriodIndex))
                {
                    CanXL_43_FLEXCAN_Ipw_MainFunction_Read(Can_pController, &((Can_apxConfig[u32PartitionId])->CanXL_pHwObjectConfig[HwObjectID]));
                }
                HwObjectID++;
            }
    #endif /* (CAN_43_FLEXCAN_USE_CANEXCEL_IP  == STD_ON) */
...
#if (CAN_43_FLEXCAN_USE_CANEXCEL_IP == STD_ON)
    #if (CAN_43_FLEXCAN_HAS_XL_HWOBJ == STD_ON)
            HwObjectID = (Can_apxConfig[u32PartitionId])->CanXL_uHthFirstIndex;
            while (HwObjectID < CAN_43_FLEXCAN_XL_HWOBJECT_CONFIG_COUNT)
            {
                u8ControllerID = (Can_apxConfig[u32PartitionId])->CanXL_pHwObjIDToCtrlIDMap[HwObjectID];
                Can_pController = (Can_apxConfig[u32PartitionId])->Can_ppController[u8ControllerID];
                if ((TRUE == Can_pController->Can_bActivation) && (TRUE == (Can_apxConfig[u32PartitionId])->CanXL_pHwObjectConfig[HwObjectID].Can_bHwObjectUsesPolling))
                {
                    CanXL_43_FLEXCAN_Ipw_MainFunction_Write(Can_pController, &((Can_apxConfig[u32PartitionId])->CanXL_pHwObjectConfig[HwObjectID]));
                }
                HwObjectID++;
            }
    #endif /* (CAN_43_FLEXCAN_HAS_XL_HWOBJ == STD_ON) */
#endif /* (CAN_43_FLEXCAN_USE_CANEXCEL_IP  == STD_ON) */
        }
    ...
    u32PartitionId = Can_GetUserId();
        ...
        if (Controller < CAN_43_FLEXCAN_CONTROLLER_CONFIG_COUNT)
        {
            ...
            else
            {
                ...
                {
                    eReturnValue = (Std_ReturnType)Can_43_FLEXCAN_Ipw_DeactivateIcomConfiguration((Can_apxConfig[u32PartitionId])->Can_ppController[Controller]);
                }
                ...
                */
                ...
                {
                    for (u8ConfigIcomIndex = 0U; u8ConfigIcomIndex < (Can_apxConfig[u32PartitionId]->u8NumCanIcomConfigs); u8ConfigIcomIndex++)
                    {
                        if (ConfigurationId == (uint8)(Can_apxConfig[u32PartitionId]->pCanIcomConfigs[u8ConfigIcomIndex].u8CanIcomConfigId))
                        {
                            if (CAN_CS_STARTED == Can_eControllerState[Controller])
                            {
                                /* ConfigID is Valid */
                                eReturnValue = (Std_ReturnType)Can_43_FLEXCAN_Ipw_SetIcomConfiguration((Can_apxConfig[u32PartitionId])->Can_ppController[Controller], &Can_apxConfig[u32PartitionId]->pCanIcomConfigs[u8ConfigIcomIndex]);
                            }
                            break;
                        }
                    }
            #if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
                    if((Can_apxConfig[u32PartitionId]->u8NumCanIcomConfigs) == u8ConfigIcomIndex)
                    {
                ...
                if((Std_ReturnType)E_OK == eReturnValue)
                {
                    if (0U != ConfigurationId)
                    {
                        Can_apxCtrlConfigIcomIndex[Controller] = &Can_apxConfig[u32PartitionId]->pCanIcomConfigs[u8ConfigIcomIndex];
                    }
                    else
                    {
                        Can_apxCtrlConfigIcomIndex[Controller] = NULL_PTR;
                    }
                    CanIf_CurrentIcomConfiguration((Can_apxConfig[u32PartitionId])->Can_ppController[Controller]->Can_u8AbstControllerID, ConfigurationId, ICOM_SWITCH_E_OK);
                }
                else
                {
                    CanIf_CurrentIcomConfiguration((Can_apxConfig[u32PartitionId])->Can_ppController[Controller]->Can_u8AbstControllerID, ConfigurationId, ICOM_SWITCH_E_FAILED);
                }
#if (CAN_43_FLEXCAN_DEV_ERROR_DETECT == STD_ON)
        #if (CAN_43_FLEXCAN_MULTIPARTITION_ENABLED == STD_ON)
            }
...
)
{
    ...
    if (NULL_PTR != Can_apxConfig[u32PartitionId])
    {
        ...
        {
#if (CAN_43_FLEXCAN_USE_CANEXCEL_IP  == STD_ON)
            Can_43_FLEXCAN_Ipw_ReportErrorState(Can_pController, CAN_ERRORSTATE_BUSOFF);
#endif
            if ((Std_ReturnType)E_OK == Can_43_FLEXCAN_Ipw_SetControllerToStopMode(Can_pController))
            {
                Can_eControllerState[u8ControllerID] = CAN_CS_STOPPED;
