...
#if (DMA_IP_MULTIPARTITION_IS_AVAILABLE == STD_ON)
    DMA_IP_STATUS_WRONG_PARTITION   = 3U,
#endif /* DMA_IP_MULTIPARTITION_IS_AVAILABLE == STD_ON */
}Dma_Ip_ReturnType;
...
/**
 * @brief  This type contains the Dma Ip Multipartition configuration Type.
 *
 * */
typedef struct
{
    const uint8 pMultiPartitionDmaChannelConfig[DMA_IP_DMA_TOTAL_NOF_LOGIC_CHANNEL];
    const uint8 pMultiPartitionDmaInstanceConfig[DMA_IP_DMA_TOTAL_NOF_LOGIC_INST];
}Dma_Ip_MultiPartitionConfigType;
...
#if (STD_ON == DMA_IP_MULTIPARTITION_IS_AVAILABLE)
    const Dma_Ip_MultiPartitionConfigType * const pxMultiPartitionConfig;
#endif /* STD_ON == DMA_IP_MULTIPARTITION_IS_AVAILABLE */
#if (STD_ON == DMA_IP_VIRTUAL_ADDRESS_MAPPING_IS_AVAILABLE)
