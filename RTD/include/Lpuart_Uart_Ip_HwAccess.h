...
#define LPUART_UART_IP_HWACCESS_SW_MAJOR_VERSION             3
...
/*!
 * @brief Get information about disabled the LPUART receiver is successful or not.
 *
 * This function gets information about disabled the LPUART receiver is successful or not.
 * Note that write 0 to this field, this field remains 1 until the receiver finishes receiving the current character
 *
 * @param Base LPUART base pointer
 */
static inline boolean Lpuart_Uart_Ip_GetReceiverCmd(const LPUART_Type * Base)
{
    boolean RetVal = FALSE;

    RetVal = (((Base->CTRL >> (uint32)(LPUART_CTRL_RE_SHIFT)) & 1U) > 0U);
    return RetVal;
}
