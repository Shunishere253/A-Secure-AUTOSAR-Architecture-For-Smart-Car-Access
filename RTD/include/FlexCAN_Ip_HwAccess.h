 ...
 */
static inline uint8 FlexCAN_GetBuffStatusFlag(const FLEXCAN_Type * pBase, uint32 MsgBuffIdx)
{
    uint32 Flag = 0U;
 ...
 */
static inline uint8 FlexCAN_GetBuffStatusImask(const FLEXCAN_Type * pBase, uint32 MsgBuffIdx)
{
    uint32 u32Imask = 0U;
...
static inline uint8 FlexCAN_GetMsgBuffIntStatusFlag(const FLEXCAN_Type * pBase, uint32 MsgBuffIdx)
{
    ...
    if (MsgBuffIdx < 32U)
    {
        ...
        Flag = (uint8)(((pBase->IFLAG1 & Mask) >> (MsgBuffIdx % 32U)) & 1U);
    }
    ...
    else if (MsgBuffIdx < 64U)
    {
        ...
        Flag = (uint8)(((pBase->IFLAG2 & Mask) >> (MsgBuffIdx % 32U)) & 1U);
    }
#if FLEXCAN_IP_FEATURE_MAX_MB_NUM <= 64U
    else
    {
    ...
    else if (MsgBuffIdx < 96U)
    {
        ...
        Flag = (uint8)(((pBase->IFLAG3 & Mask) >> (MsgBuffIdx % 32U)) & 1U);
    }
#if FLEXCAN_IP_FEATURE_MAX_MB_NUM <= 96U
    else
    {
    ...
    else
    {
        ...
        Flag = (uint8)(((pBase->IFLAG4 & Mask) >> (MsgBuffIdx % 32U)) & 1U);
    }
#endif
